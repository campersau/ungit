...
class RemotesViewModel {
  ...
  async fetch(options) {
    ...
    const fetchPromise = options.nodes
      ...
      : null;
    try {
      const result = await Promise.all([tagPromise, fetchPromise]);
  ...
  showAddRemoteDialog() {
    components
      ...
      .closeThen(async (diag) => {
        if (diag.isSubmitted()) {
          try {
            await this.server.postPromise(`/remotes/${encodeURIComponent(diag.name())}`, {
              path: this.repoPath(),
              url: diag.url(),
            });
            this.updateRemotes();
  ...
  remoteRemove(remote) {
    components
      ...
      .closeThen(async (diag) => {
        if (diag.result()) {
          try {
            await this.server.delPromise(`/remotes/${remote.name}`, { path: this.repoPath() });
            this.updateRemotes();
          } catch (e) {
            this.server.unhandledRejection(e);
          }
